...
from app.services.analytics_report_service import analytics_report_service
...
# =====================================================================
# SMART CAMPUS INSIGHTS & EXPORT ENDPOINTS
# =====================================================================

@router.get(
    "/insights",
    response_model=List[SmartInsightItem],
    summary="Get Smart Campus Insights",
    description="Returns deterministic factual observations calculated from the database according to active filters.",
)
def get_smart_insights(
    ...
    status: Optional[str] = Query(None, description="Filter by status"),
    location_id: Optional[int] = Query(None, description="Filter by location ID"),
    interval: str = Query("daily", description="Trend interval: daily, weekly, or monthly"),
    db: Session = Depends(get_db),
    _current_admin: User = Depends(require_roles(["admin"])),
):
    if interval not in VALID_INTERVALS:
    ...
    summary = analytics_service.get_combined_analytics(
        db=db,
        interval=interval,
        start_date=start_date,
        end_date=end_date,
...
@router.get(
    ...
    description="Generates a downloadable structured CSV report for the active filter parameters.",
)
def export_analytics_csv(
    ...
    csv_content = analytics_report_service.generate_csv_report(summary, filter_dict)
    today_str = date.today().strftime("%Y-%m-%d")
    filename = f"campus-analytics-{today_str}.csv"
    return Response(
        content=csv_content,
        media_type="text/csv",
        headers={
...
@router.get(
    ...
    description="Generates a downloadable professional administrative PDF report using real database metrics.",
)
def export_analytics_pdf(
    ...
    pdf_content = analytics_report_service.generate_pdf_report(summary, filter_dict)
    today_str = date.today().strftime("%Y-%m-%d")
    filename = f"campus-analytics-{today_str}.pdf"
    return Response(
        content=pdf_content,
