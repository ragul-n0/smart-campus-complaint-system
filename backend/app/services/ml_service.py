...
class ComplaintMLService:
    ...
    def _resolve_models_dir(self) -> Path:
        """Locates the ml/models directory whether running from project root or backend dir."""
        # 1. Configured path directly
        p = Path(settings.ML_MODELS_DIR)
        if p.exists() and p.is_dir():
            return p.resolve()

        # 2. Relative to workspace root from backend/app/services/
        p_root = Path(__file__).resolve().parents[3] / "ml" / "models"
        if p_root.exists() and p_root.is_dir():
            return p_root.resolve()

        # 3. Relative to backend directory
        p_parent = Path("..") / "ml" / "models"
        if p_parent.exists() and p_parent.is_dir():
