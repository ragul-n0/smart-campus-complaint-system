...
export default function AdminDepartments() {
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      setSuccessMsg(`Department '${deleteData.name}' deleted.`);
      setDeleteData(null);
      await loadData();
    } catch (err) {
      setErrorMsg(err.message || 'Failed to delete department. Verify active complaints/staff.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  return (
    <AdminLayout
      title="Campus Facility Departments"
      subtitle="Configure operational maintenance, housekeeping, electrical, IT, and safety campus units."
      action={
      ...
      <div className="bg-white border border-slate-200/90 rounded-xl shadow-card overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            ...
            <span className="text-xs text-slate-500">Loading campus departments...</span>
          </div>
        ) : departments.length === 0 ? (
          <EmptyState
            icon={Building2}
            title="No Departments Registered"
            description="Create your first campus facility department to organize complaints and staff."
                ...
                {departments.map((dept) => (
                  <tr key={dept.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-600 max-w-sm">
                      {dept.description || <span className="text-slate-400 italic">None provided</span>}
                    </td>

                    <td className="py-3.5 px-4">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-slate-700 font-mono text-[11px]">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-xl max-w-md w-full p-6 shadow-elevated animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-900">Add New Campus Department</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Electrical, Transport, Library..."
                  value={formData.name}
              ...
              <div>
                ...
                <textarea
                  placeholder="Scope of responsibilities and maintenance coverage..."
      ...
      {deleteData && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-xl max-w-sm w-full p-6 shadow-elevated animate-in fade-in zoom-in-95">
            ...
            <h3 className="text-base font-bold text-slate-900 mb-1">Delete Department?</h3>
            <p className="text-xs text-slate-600 mb-4">
              Are you sure you want to delete{' '}
              <strong className="text-slate-900">{deleteData.name}</strong>?
            </p>
