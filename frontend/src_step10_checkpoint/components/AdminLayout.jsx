import React from 'react';
import { NavLink } from 'react-router-dom';
import { LayoutDashboard, AlertCircle, Users, Building2, MapPin, BarChart3, ShieldCheck } from 'lucide-react';
import { getUser } from '../services/auth';

export default function AdminLayout({ children, title, subtitle, action }) {
  const user = getUser();

  const navItems = [
    { to: '/admin/dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { to: '/admin/complaints', label: 'Complaints', icon: AlertCircle },
    { to: '/admin/users', label: 'Users', icon: Users },
    { to: '/admin/departments', label: 'Departments', icon: Building2 },
    { to: '/admin/locations', label: 'Locations', icon: MapPin },
    { to: '/admin/reports', label: 'Reports', icon: BarChart3 },
  ];

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-1 w-full">
      {/* Admin Top Header Card */}
      <div className="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-5 sm:p-6 shadow-xl backdrop-blur-md mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-500/10 text-purple-300 border border-purple-500/30 mb-2">
              <ShieldCheck className="w-3.5 h-3.5 text-purple-400" />
              Central Campus Administration
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
              {title || 'Admin Management Console'}
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              {subtitle || `Logged in as ${user?.name || 'Administrator'} (@${user?.username})`}
            </p>
          </div>

          {action && (
            <div className="flex items-center gap-3">
              {action}
            </div>
          )}
        </div>

        {/* Subnavigation Bar */}
        <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto pt-5 mt-5 border-t border-slate-700/60 no-scrollbar">
          {navItems.map((item) => {
            const Icon = item.icon;
            return (
              <NavLink
                key={item.to}
                to={item.to}
                className={({ isActive }) =>
                  `flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${
                    isActive
                      ? 'bg-purple-600 text-white shadow-md shadow-purple-600/30 font-semibold'
                      : 'text-slate-300 hover:text-white hover:bg-slate-700/60'
                  }`
                }
              >
                <Icon className="w-4 h-4" />
                <span>{item.label}</span>
              </NavLink>
            );
          })}
        </div>
      </div>

      {/* Main Content Area */}
      <div className="w-full">
        {children}
      </div>
    </div>
  );
}
