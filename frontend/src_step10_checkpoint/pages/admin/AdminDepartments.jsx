...
export default function AdminDepartments() {
  ...
  const [deleteData, setDeleteData] = useState(null); // department to delete
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to load campus departments.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(`Department '${editData.name}' updated successfully.`);
      setEditData(null);
      await loadData();
    } catch (err) {
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      setSuccessMsg(`Department '${deleteData.name}' deleted successfully.`);
      setDeleteData(null);
      await loadData();
    } catch (err) {
  ...
  return (
    <AdminLayout
      ...
      subtitle="Configure campus departments, supervise assigned issue workloads, and staff teams."
      action={
    ...
    >
      ...
      <div className="bg-slate-800/80 border border-slate-700/80 rounded-2xl shadow-xl overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            ...
            <span className="text-sm">Loading campus departments...</span>
          </div>
        ) : departments.length === 0 ? (
          <div className="py-16 px-4 text-center">
            <Building2 className="w-10 h-10 text-slate-500 mx-auto mb-3" />
            <h3 className="text-base font-bold text-white mb-1">No Departments Registered</h3>
            <p className="text-slate-400 text-xs">Create your first campus department above.</p>
          </div>
        ) : (
                ...
                {departments.map((dept) => (
                  <tr key={dept.id} className="hover:bg-slate-700/30 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-300 max-w-sm">
                      {dept.description || <span className="text-slate-500 italic">None provided</span>}
                    </td>

                    <td className="py-3.5 px-4">
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 font-mono text-[11px]">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-300 font-mono text-[11px]">
                        <FileText className="w-3 h-3" />
                        <span>{dept.complaint_count} issues</span>
                      </div>
                    </td>
      ...
      {isAddOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-md w-full p-6 shadow-2xl animate-in fade-in zoom-in-95">
            <h3 className="text-base font-bold text-white mb-3">Add New Campus Department</h3>
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Sports, Transport, Library..."
                  value={formData.name}
      ...
      {editData && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-md w-full p-6 shadow-2xl animate-in fade-in zoom-in-95">
            <h3 className="text-base font-bold text-white mb-3">Edit Department #{editData.id}</h3>
      ...
      {deleteData && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl animate-in fade-in zoom-in-95">
            ...
            <h3 className="text-base font-bold text-white mb-1">Delete Department?</h3>
            ...
            <div className="bg-slate-900/60 border border-slate-700/60 rounded-xl p-3 mb-4 text-[11px] text-slate-400">
              Note: Departments currently associated with active complaints or assigned staff cannot be deleted.
            </div>
