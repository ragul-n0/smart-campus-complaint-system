...
Pipeline:
  complaint_text -> TF-IDF Vectorizer -> Logistic Regression -> priority (Low, Medium, High)
"""
...
def train_priority_model(
    ...
    if output_model_path is None:
        output_model_path = base_dir / "models" / "priority_model.joblib"
    if output_cm_path is None:
        output_cm_path = base_dir / "models" / "priority_confusion_matrix.png"
    ...
    # 2. Validate columns
    required_cols = ["complaint_text", "priority"]
    ...
    # Drop any rows with empty text or priority
    ...
    # - ngram_range=(1, 2): captures unigrams and bigrams (e.g., 'severely leaking', 'completely dark')
    ...
    plt.title("Priority Model Confusion Matrix", fontsize=14, pad=15)
    plt.colorbar()
    ...
    plt.ylabel("Actual Priority", fontsize=12, fontweight="bold")
    plt.xlabel("Predicted Priority", fontsize=12, fontweight="bold")
    plt.tight_layout()
    ...
    print(f"Saved trained priority model to: {output_model_path}")
