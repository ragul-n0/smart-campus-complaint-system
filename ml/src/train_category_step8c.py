"""
Step 8C Category Classification Model Training & Fair Evaluation.

Trains on ml/data/complaint_dataset_step8c.csv (920 samples).
Saves model artifact to ml/models/category_model_step8c.joblib
Saves confusion matrix to ml/models/category_confusion_matrix_step8c.png
Preserves ml/models/category_model.joblib as Step 8B baseline.
"""
...
def train_category_step8c(
    ...
    if output_model_path is None:
        output_model_path = base_dir / "models" / "category_model_step8c.joblib"
    if output_cm_path is None:
        output_cm_path = base_dir / "models" / "category_confusion_matrix_step8c.png"

    print("=" * 65)
    print("STEP 8C — Category Model Training on Improved Dataset")
    print("=" * 65)
    ...
    print("\nFitting Step 8C Pipeline on training data...")
    ...
    print("\n" + "=" * 50)
    print("STEP 8C EVALUATION RESULTS (Test Set):")
    print("=" * 50)
    ...
    plt.title("Step 8C Category Confusion Matrix (N=184 Test Samples)", fontsize=13, pad=15)
    plt.colorbar()
    ...
    plt.close()
    print(f"Saved Step 8C confusion matrix to: {output_cm_path}")

    # Save Step 8C Model separately
    output_model_path.parent.mkdir(parents=True, exist_ok=True)
    joblib.dump(pipeline, output_model_path)
    print(f"Saved Step 8C category model to: {output_model_path}")
